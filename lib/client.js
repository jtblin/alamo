  ...
  function parseStatus (res, cb) {
    ...
    parseBody(res, function (err, res) {
      ...
      parseXMLString(res.body, function (err, result) {
        var body;
        if (err) {
          debug('Error parsing error body', err.message);
          return cb(error);
        }
        if (! result) {
          debug('Error parsing error body');
          return cb(error);
        }
        body = result.Error;
        error.code = body.Code && body.Code.length && body.Code[0];
        error.message = body.Message && body.Message.length && body.Message[0];
        error.requestId = body.RequestId && body.RequestId.length && body.RequestId[0];
        error.hostId = body.HostId && body.HostId.length && body.HostId[0];
        error.key = body.Key && body.Key.length && body.Key[0];
        cb(error);
      });
    });
  }
