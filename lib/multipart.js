    ...
    function initiateUpload () {
      self.post(format('%s?uploads', filename), '', headers, options, function (err, res) {
        ...
        parseXMLString(res.body, function (err, result) {
          var body;
          if (err) return ws.emit('error', err);
          if (! result) return ws.emit('error', new Error('Could not parse XML string'));
          body = result.InitiateMultipartUploadResult;
          if (! body || ! body.UploadId[0]) return ws.emit('error', new Error('Invalid upload id'));
          ws.uploadId = body.UploadId[0];
